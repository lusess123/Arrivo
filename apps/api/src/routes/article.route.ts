...
import {
  ...
  recordSentenceWordPlayInputSchema,
...
export function registerArticleRoutes(app: Hono<AppEnv>, prefix = "") {
  ...
  app.post(
    route(prefix, "/article/recordSentenceWordPlay"),
    requireUser,
    zValidator("json", recordSentenceWordPlayInputSchema),
    async (c) => {
      const user = c.get("user");
      if (!user) throw httpError.unauthorized();
      const input = c.req.valid("json");
      return ok(c, await recordSentenceWordPlay({
        userId: user.id,
        tenantId: user.tenant,
        id: input.id,
  ...
  app.post(
    route(prefix, "/articles/:articleId/sentences/:sentenceId/regenerate-stream"),
    requireUser,
    zValidator("param", sentenceSplitParamSchema),
    zValidator("json", sentenceRegenerateInputSchema),
    async (c) => {
      const user = c.get("user");
      if (!user) throw httpError.unauthorized();
      const { articleId, sentenceId } = c.req.valid("param");
